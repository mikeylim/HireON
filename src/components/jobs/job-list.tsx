		...
		switch (job.status) {
			case "interview":
				if (job.interview_date) {
					return {
						icon: CalendarClock,
						text: `Interview ${relativeTime(job.interview_date)}`,
					};
				}
				break;
			case "offer":
				if (job.offer_date) {
					return {
						icon: CalendarDays,
						text: `Offer received ${relativeTime(job.offer_date)}`,
					};
				}
				break;
			case "applied":
				if (job.applied_date) {
					return {
						icon: CalendarDays,
						text: `Applied ${relativeTime(job.applied_date)}`,
					};
				}
				break;
			case "rejected":
				if (job.rejected_date) {
					return {
						icon: CalendarDays,
						text: `Rejected ${relativeTime(job.rejected_date)}`,
					};
				}
				break;
			case "archived":
				if (job.archived_date) {
					return {
						icon: CalendarDays,
						text: `Archived ${relativeTime(job.archived_date)}`,
					};
				}
				break;
			case "saved":
				...
				<div className="flex flex-wrap items-center gap-1">
					{sortOptions.map((opt) => (
						<button
							...
							{opt.label}
							{sortField === opt.field && <ArrowUpDown className="h-3 w-3" />}
						</button>
					))}
									...
									<div className="flex min-w-0 flex-1 items-start justify-between gap-3">
										<div className="min-w-0 flex-1">
											...
											<p className="truncate text-sm text-[var(--muted)]">
												{job.company} <span className="opacity-50">·</span>{" "}
												{job.location}
